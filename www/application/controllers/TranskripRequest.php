...
class TranskripRequest extends CI_Controller {
    ...
    public function add() {
        try {
            ...
            $injection = "insert into Transkrip(requestByEmail,requestDateTime,requestType,requestUsage) values ('" . $userInfo['email'] . "','" . strftime('%Y-%m-%d %H:%M:%S') . "','" . $requestType . "','" . $_POST['requestUsage'] . "')";

            $serverName = "localhost";
            $username = "root";
            $password = "";

            $conn = new mysqli($serverName, $username, $password, "bluetape");
            ...
            $result = mysqli_multi_query($conn, $injection);
